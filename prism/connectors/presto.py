...
class PrestoConnector(Connector):
    ...
    # This should be an instance of the `prestodb.dbapi.Connection`, but we don't want
    # to import prestodb unless the user creates calls the `create_engine` method.
    engine: Any
    ...
    def create_engine(self) -> Any:
        """
        Create the PrestoDB connection
        """
        import prestodb
    ...
    def execute_sql(
        ...
        cursor: prestodb.dbapi.Cursor = self.engine.cursor()
        cursor.execute(sql)
