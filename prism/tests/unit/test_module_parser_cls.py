"""
Unit testing for parsing models with tasks created via the PrismTask class.
...
class TestModelClassParsing(unittest.TestCase):
    ...
    def test_multiple_prism_task(self):
        ...
        """
        parser = ast_parser.AstParser(MULTIPLE_PRISM_TASKS, MODEL_TEST_CASES)
    ...
    def test_if_name_main(self):
        """
        If a model contains `if __name__ == '__main__'`, throw an error
        """
        with self.assertRaises(prism.exceptions.ParserException) as cm:
            _ = ast_parser.AstParser(IF_NAME_MAIN, MODEL_TEST_CASES)
    ...
    def test_bad_runs(self):
        ...
        for model in [
            ...
            with self.assertRaises(prism.exceptions.ParserException) as cm:
                parser = ast_parser.AstParser(model, MODEL_TEST_CASES)
                parser.parse()
            if model == NO_RUN_FUNC:
                expected_msg = f"no `run` function in PrismTask in `{str(model)}`"
            else:
                expected_msg = f'invalid arguments in `run` function in PrismTask in {str(model)}; should only be `self`,`tasks`,`hooks`'  # noqa: E501
            self.assertEqual(expected_msg, str(cm.exception))
